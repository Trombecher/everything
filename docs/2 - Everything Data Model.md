...
* The _knowledge_ is a composite object which is a parameter to each expression, a sort of context which influences evaluation.

* An object _has a tag with a value_ iff there exists a statement in the knowledge that includes the object, the tag, and the value; or, if the object is a composite object, `tag = value` is a property on object.
...
Everything constructs the integers recursively through nested composite objects:
...
By repeatetly constructing composite objects as successors or predecessors, you can get any integer. An integer is _negative_ iff it has `$PREDECESSOR_OF`. An integer is positive iff it has `$SUCCESSOR_OF`. An integer must not have both.
...
Function nodes are defined by `$FUNCTION`. The associated value can be interpreted as the function body. If the function is called with a parameter, then the function body will be evaluated with the parameter value.
...
| Composite Object                                          | Non-normative Textual Representation         |
|-----------------------------------------------------------|----------------------------------------------|
...
| Composite Object                                              | Domain    | Meaning                                             |
|---------------------------------------------------------------|-----------|-----------------------------------------------------|
...
There are nodes to query the knowledge. For each query, first the composite properties of the subject are queries (abstract objects do not have composite properties) and then the whole knowledge is queried. Queries always return sets.
...
function $subject =>
    function $value =>
        ...
        and ($value == 0 or query ($value, $SUCCESSOR_OF, ?))
```
