...
export class Database {
    ...
    public static tryFrom(
        ...
        const set = new Set(
            associations[Symbol.iterator]().map(packAssociation),
        );
    ...
    private validate(): ValidationResult {
        if (this.unsafeFirstValue(M_TAG, M_UNIQUE) === undefined)
            return {
                type: "missing",
                form: [M_TAG, M_UNIQUE, "some"],
            };
        ...
        for (const [object, tag, value] of this.iterStored()) {
            ...
            if (this.unsafeFirstValue(value, valueTag) === undefined)
                return {
                    type: "missing",
                    form: [value, valueTag, "some"],
                };
        }
        ...
        for (const [tag, _] of this.iterStoredObjectsAndValues(M_UNIQUE)) {
            for (const object of this.iterStoredObjects(tag)) {
                const secondValue = this.iterValues(object, tag)
                    .drop(1)
                    .next().value;
        ...
        for (const [tag, _f] of this.iterStoredObjectsAndValues(M_INFERRED)) {
            ...
            if (storedObjectAndValue !== undefined)
                return {
                    type: "found",
                    form: [
                        storedObjectAndValue[0],
                        tag,
                        storedObjectAndValue[1],
                    ],
                };
        }
    ...
    public modify(modifications: Modification[]): ValidationResult {
    ...
    private match(object: ObjectId, tag: ObjectId): ValidationResult {
        for (const requiredTag of this.iterStoredValues(tag, M_REQUIRES)) {
            if (this.unsafeFirstValue(object, requiredTag) === undefined)
                return {
                    type: "missing",
                    form: [object, requiredTag, "some"],
                };
        }

        for (const requiredNotTag of this.iterStoredValues(
            tag,
            M_REQUIRES_NOT,
        )) {
            if (this.unsafeFirstValue(object, requiredNotTag) !== undefined)
                return {
                    type: "found",
                    form: [object, requiredNotTag, "some"],
                };
        }

        // Or constraints
        {
            let some: undefined | Reason | true;

            for (const requiredOrTag of this.iterStoredValues(
                tag,
                M_REQUIRES_OR,
            )) {
                const hasTag =
                    this.unsafeFirstValue(object, requiredOrTag) !== undefined;

                if (!hasTag) {
                    some = {
                        type: "missing",
                        form: [object, requiredOrTag, "some"],
                    };
                } else {
                    some = true;
                    break;
            ...
            if (some !== true && some !== undefined) return some;
        }
        ...
        // Or not constraints
        {
            ...
            for (const requiredOrNotTag of this.iterStoredValues(
                tag,
                M_REQUIRES_OR_NOT,
            )) {
                const hasTag =
                    this.unsafeFirstValue(object, requiredOrNotTag) !==
                    undefined;

                if (hasTag) {
                    some = {
                        type: "found",
                        form: [object, requiredOrNotTag, "some"],
                    };
                } else {
                    some = true;
                    break;
    ...
    public firstStoredValue(object: ObjectId, tag: ObjectId) {
        return this.iterStoredValues(object, tag).next().value;
    }

    private unsafeFirstValue(object: ObjectId, tag: ObjectId) {
        const f = this.tagInferredValue(tag);
    ...
    public firstValue(object: ObjectId, tag: ObjectId): ObjectId | undefined {
        const f = this.tagInferredValue(tag);

        if (f !== undefined) {
            // `tag` is inferred, so we need to check if `tag`
            // is applicable to `object`:

            if (this.match(object, tag) !== true) return;

            // Now that it matches, we can compute the value:

            return this.compute(f, object);
        }
