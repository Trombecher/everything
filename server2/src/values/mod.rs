...
use crate::{constraints::{Constraint, EncodedConstraint}, decode::{Decodable, PartiallyDecodable}, ff, schema::{EncodedSchema, Schema}, ObjectId};
...
#[inline]
unsafe fn read_bytes<const N: usize>(slice: &[u8], offset: usize) -> [u8; N] {
...
impl EncodedValue {
    ...
    fn validate(slice: &[u8]) -> bool {
        match slice.get(0).copied() {
            ...
            Some(ff::value::INTEGER | ff::value::FLOAT | ff::value::OBJECT) if slice.len() >= 9 => true,
    ...
    fn decode_partial(&self) -> Self::PartialOutput {
        match self.0.get(0).copied() {
            ...
            Some(ff::value::CHARACTER) => unsafe {
                PartiallyDecodedValue::Character(char::from_u32_unchecked(
                    u32::from_le_bytes(read_bytes::<4>(&self.0, 1))
                ))
            }
            Some(ff::value::DURATION) => unsafe {
                PartiallyDecodedValue::Duration(Duration::from_nanos(
                    i128::from_le_bytes(read_bytes::<16>(&self.0, 1))
                ))
            }
            Some(ff::value::DATE_TIME) => unsafe {
                PartiallyDecodedValue::DateTime(DateTime::from(
                    i128::from_le_bytes(read_bytes::<16>(&self.0, 1))
                ))
            }
            // TODO: more
